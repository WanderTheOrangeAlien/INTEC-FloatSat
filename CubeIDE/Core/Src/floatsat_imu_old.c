 ...
 * @param hi2c           I2C peripheral.
 * @param address_7bit   Device address.
 * @param first_register First register to read.
 * @param data           Array to write the data to.
        ...
        uint16_t length)
{
    ...
    // When sending the register address, setting the 8th bit will
    // activate autoincrement. Subequent reads will get consecutive registers
    register_address = (uint8_t)(first_register | FS_IMU_AUTO_INCREMENT);
 ...
 *  @param address_count Amount of addresses to test
 *  @param expected_who_am_i
 *  @param detected_address
 *  @param detected_who_am_i
 * 
 * @return
        ...
        uint8_t *detected_who_am_i)
{
    ...
    if ((hi2c == NULL) || (addresses == NULL) || (detected_address == NULL) ||
    ...
    *detected_who_am_i = 0U;
    ...
    {
        // Check for response in the current address
        ...
        who_am_i = 0U;
        status = FS_IMU_ReadRegister( hi2c,addresses[index], FS_IMU_REG_WHO_AM_I, &who_am_i);

        if (status != HAL_OK){
            continue;
        }

        // Verify expected who_am_i
    ...
    // No device with expected who_am_i was detected
    return 0U;
}
